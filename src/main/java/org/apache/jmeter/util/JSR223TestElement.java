...
public abstract class JSR223TestElement extends ScriptingTestElement
    ...
    protected Object processFileOrScript(ScriptEngine scriptEngine, final Bindings pBindings)
        ...
        if (scriptEngine instanceof GroovyScriptEngineImpl) {
            try {
                ...
                loadGroovyJar(groovyScriptEngine.getClassLoader());
            } catch (Exception e) {
    ...
    public void loadGroovyJar(GroovyClassLoader classLoader) throws Exception {
        LoggerUtil.info("开始加载对应GroovyJar ");
        File file = new File(FileUtils.JAR_FILE_DIR);
        if (file.isFile()) {
            classLoader.addURL(file.toURI().toURL());
            LoggerUtil.info("加载对应GroovyJar完成");
        } else {
            ...
            if (ArrayUtils.isNotEmpty(files)) {
                ...
                LoggerUtil.info("加载对应GroovyJar完成", files.length);
            }
        }
    }
